...
export default function Dashboard({ smallStats, ...props }) {
  ...
  useEffect(() => {
    setTimeout(() => {
      adminApi.getStatistics({ type: 'crawling-time'}).then(resp => {
        if (resp?.success) {
          setCrawlingTimeShopee(resp.data.shopee);
          setCrawlingTimeTiki(resp.data.tiki);
        }
    ...
    <Container fluid className="main-content-container px-4">
      ...
      <Row>
        ...
        <Col lg="8" md="12" sm="12" className="mb-4">
          <CrawlingTimeOverview dataTiki={crawlingTimeTiki} dataShopee={crawlingTimeShopee}/>
        </Col>
